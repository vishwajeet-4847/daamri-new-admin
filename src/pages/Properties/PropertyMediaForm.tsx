...
import { useUpdatePropertyMedia, useDeletePropertyMedia, useProperty } from '../../services/api';
...
export default function PropertyMediaForm({ propertyId, onClose }: PropertyMediaFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateMedia.mutateAsync({ id: propertyId, formData });
      setCoverImage(null);
      setGallery(null);
      setBrochure(null);
      setVideo(null);
      ...
      >
        <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900">Manage Property Media</h3>
              <p className="text-xs text-slate-500 font-medium">Update images, brochures, and videos</p>
            </div>
          </div>
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-8">
          ...
          <div className="space-y-4">
            <h4 className="text-sm font-bold text-slate-900 border-b border-slate-100 pb-2">Current Media</h4>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {property?.media?.coverImage && (
                <div className="relative aspect-video rounded-xl overflow-hidden group border border-slate-200">
                  <img src={property.media.coverImage} className="w-full h-full object-cover" alt="Cover" />
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
              ...
              <button 
                type="submit" 
                disabled={updateMedia.isPending || !coverImage && !gallery && !brochure && !video}
                className="px-6 py-2 rounded-xl bg-emerald-600 text-white text-sm font-bold hover:bg-emerald-700 transition-colors disabled:opacity-50"
              >
